...
def update_package(revamp_dir: Path):
    ...
    run_process("unset VIRTUAL_ENV && poetry run which python", cwd=revamp_dir)
    run_process("unset VIRTUAL_ENV && poetry env info", cwd=revamp_dir)
    run_process("unset VIRTUAL_ENV && poetry env list --full-path", cwd=revamp_dir)
